...
const AP_Param::GroupInfo AC_AttitudeControl::var_info[] = {
    ...
    // @Increment: 100
    // @User: Advanced
    AP_GROUPINFO("SLEW_YAW", 2, AC_AttitudeControl, _slew_yaw,
		AC_ATTITUDE_CONTROL_SLEW_YAW_DEFAULT_CDS)
,
    ...
    // @Increment: 1000
    // @User: Advanced
    AP_GROUPINFO("ACCEL_Y_MAX", 4, AC_AttitudeControl, _accel_yaw_max,
		AC_ATTITUDE_CONTROL_ACCEL_Y_MAX_DEFAULT_CDSS)
,
    ...
    // @User: Advanced
    AP_GROUPINFO("RATE_FF_ENAB", 5, AC_AttitudeControl, _rate_bf_ff_enabled,
...
,
    ...
    // @User: Advanced
    AP_GROUPINFO("ACCEL_R_MAX", 6, AC_AttitudeControl, _accel_roll_max,
		AC_ATTITUDE_CONTROL_ACCEL_RP_MAX_DEFAULT_CDSS)
,
    ...
    // @Values: 0:Disabled, 72000:Slow, 108000:Medium, 162000:Fast
    // @User: Advanced
    AP_GROUPINFO("ACCEL_P_MAX", 7, AC_AttitudeControl, _accel_pitch_max,
		AC_ATTITUDE_CONTROL_ACCEL_RP_MAX_DEFAULT_CDSS)
,
...
,
    ...
    // @Range: 0.5 10.0
    // @User: Advanced
    AP_GROUPINFO("ANG_LIM_TC", 16, AC_AttitudeControl, _angle_limit_tc,
...
,
    ...
    // @Range: 0 40
    // @User: Advanced
    AP_GROUPINFO("SWEEP_FADEI", 23, AC_AttitudeControl, _sweep_fadein, 1.0f)
,
    ...
    // @Range: 0 40
    // @User: Advanced
    AP_GROUPINFO("SWEEP_FADEO", 24, AC_AttitudeControl, _sweep_fadeot, 1.0f)
,
...
void AC_AttitudeControl::set_throttle_out_unstabilized(float throttle_in,
	bool reset_attitude_control, float filter_cutoff) {
...
_attitude_target_quat.to_euler(_attitude_target_euler_angle.x,
		_attitude_target_euler_angle.y, _attitude_target_euler_angle.z);

// ensure smoothing gain can not cause overshoot
smoothing_gain = constrain_float(smoothing_gain, 1.0f, 1 / _dt);

Quaternion attitude_error_quat = _attitude_target_quat.inverse()
...
if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
	...
	_attitude_target_ang_vel.x = input_shaping_angle(attitude_error_angle.x,
			smoothing_gain, get_accel_roll_max_radss(),
			_attitude_target_ang_vel.x);
	_attitude_target_ang_vel.y = input_shaping_angle(attitude_error_angle.y,
			smoothing_gain, get_accel_pitch_max_radss(),
			_attitude_target_ang_vel.y);
	_attitude_target_ang_vel.z = input_shaping_angle(attitude_error_angle.z,
			smoothing_gain, get_accel_yaw_max_radss(),
			_attitude_target_ang_vel.z);

	// Convert body-frame angular velocity into euler angle derivative of desired attitude
	ang_vel_to_euler_rate(_attitude_target_euler_angle,
			_attitude_target_ang_vel, _attitude_target_euler_rate);
} else {
...
void AC_AttitudeControl::input_euler_angle_roll_pitch_euler_rate_yaw(
	float euler_roll_angle_cd, float euler_pitch_angle_cd,
	float euler_yaw_rate_cds, float smoothing_gain) {
...
if (_sweep_input == 1) {
	if (_sweep_axis == 1) {
		_attitude_target_euler_angle.x = _attitude_target_euler_angle.x
				+ _sweep_output;
	} else if (_sweep_axis == 2) {
		_attitude_target_euler_angle.y = _attitude_target_euler_angle.y
				+ _sweep_output;
	} else if (_sweep_axis == 3) {
		_attitude_target_euler_angle.z = _attitude_target_euler_angle.z
				+ _sweep_output;
	}
}
...
if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
	...
	Vector3f euler_accel = euler_accel_limit(_attitude_target_euler_angle,
			Vector3f(get_accel_roll_max_radss(), get_accel_pitch_max_radss(),
	...
	_attitude_target_euler_rate.x = input_shaping_angle(
			euler_roll_angle - _attitude_target_euler_angle.x, smoothing_gain,
			euler_accel.x, _attitude_target_euler_rate.x);
	_attitude_target_euler_rate.y = input_shaping_angle(
			euler_pitch_angle - _attitude_target_euler_angle.y, smoothing_gain,
			euler_accel.y, _attitude_target_euler_rate.y);

	// When yaw acceleration limiting is enabled, the yaw input shaper constrains angular acceleration about the yaw axis, slewing
	// the output rate towards the input rate.
	_attitude_target_euler_rate.z = input_shaping_ang_vel(
			_attitude_target_euler_rate.z, euler_yaw_rate, euler_accel.z);

	// Convert euler angle derivative of desired attitude into a body-frame angular velocity vector for feedforward
	euler_rate_to_ang_vel(_attitude_target_euler_angle,
			_attitude_target_euler_rate, _attitude_target_ang_vel);
} else {
	...
	_attitude_target_quat.from_euler(_attitude_target_euler_angle.x,
			_attitude_target_euler_angle.y, _attitude_target_euler_angle.z);
...
void AC_AttitudeControl::input_euler_angle_roll_pitch_yaw(
	float euler_roll_angle_cd, float euler_pitch_angle_cd,
	float euler_yaw_angle_cd, bool slew_yaw, float smoothing_gain) {
// Convert from centidegrees on public interface to radians
float euler_roll_angle = radians(euler_roll_angle_cd * 0.01f);
float euler_pitch_angle = radians(euler_pitch_angle_cd * 0.01f);
...
if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
	...
	_attitude_target_euler_rate.y = input_shaping_angle(
			euler_pitch_angle - _attitude_target_euler_angle.y, smoothing_gain,
			euler_accel.y, _attitude_target_euler_rate.y);
	_attitude_target_euler_rate.z = input_shaping_angle(
			euler_yaw_angle - _attitude_target_euler_angle.z, smoothing_gain,
			euler_accel.z, _attitude_target_euler_rate.z);
	if (slew_yaw) {
		_attitude_target_euler_rate.z = constrain_float(
				_attitude_target_euler_rate.z, -get_slew_yaw_rads(),
				get_slew_yaw_rads());
	}
	...
	if (slew_yaw) {
		...
		float angle_error = constrain_float(
				wrap_PI(euler_yaw_angle - _attitude_target_euler_angle.z),
				-get_slew_yaw_rads() * _dt, get_slew_yaw_rads() * _dt);
		// Update attitude target from constrained angle error
		_attitude_target_euler_angle.z = wrap_PI(
				angle_error + _attitude_target_euler_angle.z);
	} else {
...
if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
	...
	// the output rate towards the input rate.
	_attitude_target_euler_rate.x = input_shaping_ang_vel(
			_attitude_target_euler_rate.x, euler_roll_rate, euler_accel.x);
	_attitude_target_euler_rate.y = input_shaping_ang_vel(
			_attitude_target_euler_rate.y, euler_pitch_rate, euler_accel.y);
	_attitude_target_euler_rate.z = input_shaping_ang_vel(
			_attitude_target_euler_rate.z, euler_yaw_rate, euler_accel.z);
	...
	_attitude_target_euler_angle.x = wrap_PI(
			_attitude_target_euler_angle.x + euler_roll_rate * _dt);
	_attitude_target_euler_angle.y = constrain_float(
			_attitude_target_euler_angle.y + euler_pitch_rate * _dt,
			radians(-85.0f), radians(85.0f));
	_attitude_target_euler_angle.z = wrap_2PI(
			_attitude_target_euler_angle.z + euler_yaw_rate * _dt);
...
void AC_AttitudeControl::input_rate_bf_roll_pitch_yaw(float roll_rate_bf_cds,
	float pitch_rate_bf_cds, float yaw_rate_bf_cds) {
// Convert from centidegrees on public interface to radians
...
if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
	...
	// the output rate towards the input rate.
	_attitude_target_ang_vel.x = input_shaping_ang_vel(
			_attitude_target_ang_vel.x, roll_rate_rads,
			get_accel_roll_max_radss());
	_attitude_target_ang_vel.y = input_shaping_ang_vel(
			_attitude_target_ang_vel.y, pitch_rate_rads,
			get_accel_pitch_max_radss());
	_attitude_target_ang_vel.z = input_shaping_ang_vel(
	...
	attitude_target_update_quat.from_axis_angle(
			Vector3f(roll_rate_rads * _dt, pitch_rate_rads * _dt,
					yaw_rate_rads * _dt));
...
void AC_AttitudeControl::input_angle_step_bf_roll_pitch_yaw(
	float roll_angle_step_bf_cd, float pitch_angle_step_bf_cd,
...
Quaternion attitude_target_update_quat;
attitude_target_update_quat.from_axis_angle(
		Vector3f(roll_step_rads, pitch_step_rads, yaw_step_rads));
_attitude_target_quat = _attitude_target_quat * attitude_target_update_quat;
_attitude_target_quat.normalize();
...
_rate_target_ang_vel = update_ang_vel_target_from_att_error(
...
Quaternion attitude_target_ang_vel_quat = Quaternion(0.0f,
		_attitude_target_ang_vel.x, _attitude_target_ang_vel.y,
		_attitude_target_ang_vel.z);
Quaternion attitude_error_quat = attitude_vehicle_quat.inverse()
		* _attitude_target_quat;
Quaternion target_ang_vel_quat = attitude_error_quat.inverse()
		* attitude_target_ang_vel_quat * attitude_error_quat;

// Correct the thrust vector and smoothly add feedforward and yaw input
if (_thrust_error_angle > AC_ATTITUDE_THRUST_ERROR_ANGLE * 2.0f) {
	_rate_target_ang_vel.z = _ahrs.get_gyro().z;
} else if (_thrust_error_angle > AC_ATTITUDE_THRUST_ERROR_ANGLE) {
	float flip_scalar = (1.0f
			- (_thrust_error_angle - AC_ATTITUDE_THRUST_ERROR_ANGLE)
	...
	_rate_target_ang_vel.z = _ahrs.get_gyro().z * (1.0 - flip_scalar)
			+ _rate_target_ang_vel.z * flip_scalar;
} else {
...
if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
	...
	Quaternion attitude_target_update_quat;
	attitude_target_update_quat.from_axis_angle(
			Vector3f(_attitude_target_ang_vel.x * _dt,
					_attitude_target_ang_vel.y * _dt,
					_attitude_target_ang_vel.z * _dt));
	_attitude_target_quat = _attitude_target_quat * attitude_target_update_quat;
	_attitude_target_quat.normalize();
}
}
...
void AC_AttitudeControl::thrust_heading_rotation_angles(Quaternion& att_to_quat,
	const Quaternion& att_from_quat, Vector3f& att_diff_angle,
...
thrust_vec_dot = acosf(
		constrain_float(att_from_thrust_vec * att_to_thrust_vec, -1.0f, 1.0f));
...
thrust_vec_correction_quat = att_from_quat.inverse()
		* thrust_vec_correction_quat * att_from_quat;

// calculate the remaining rotation required after thrust vector is rotated
Quaternion heading_quat = thrust_vec_correction_quat.inverse()
		* att_from_quat.inverse() * att_to_quat;
...
if (!is_zero(_p_angle_yaw.kP())
		&& fabsf(att_diff_angle.z)
				> AC_ATTITUDE_ACCEL_Y_CONTROLLER_MAX_RADSS
						/ _p_angle_yaw.kP()) {
	att_diff_angle.z = constrain_float(wrap_PI(att_diff_angle.z),
			-AC_ATTITUDE_ACCEL_Y_CONTROLLER_MAX_RADSS / _p_angle_yaw.kP(),
			AC_ATTITUDE_ACCEL_Y_CONTROLLER_MAX_RADSS / _p_angle_yaw.kP());
...
float AC_AttitudeControl::input_shaping_angle(float error_angle,
	float smoothing_gain, float accel_max, float target_ang_vel) {
...
if (accel_max > 0) {
	...
	return constrain_float(ang_vel, target_ang_vel - delta_ang_vel,
...
if (accel_max > 0.0f) {
	float delta_ang_vel = accel_max * _dt;
	target_ang_vel += constrain_float(desired_ang_vel - target_ang_vel,
			-delta_ang_vel, delta_ang_vel);
} else {
...
Vector3f rot_accel;
if (is_zero(euler_accel.x) || is_zero(euler_accel.y) || is_zero(euler_accel.z)
		|| (euler_accel.x < 0.0f) || (euler_accel.y < 0.0f)
		|| (euler_accel.z < 0.0f)) {
	...
	rot_accel.z = MIN(MIN(euler_accel.x / sin_theta, euler_accel.y / sin_phi),
			euler_accel.z / cos_phi);
}
return rot_accel;
}
...
ang_vel_rads.y = cos_phi * euler_rate_rads.y
		+ sin_phi * cos_theta * euler_rate_rads.z;
ang_vel_rads.z = -sin_phi * euler_rate_rads.y
		+ cos_theta * cos_phi * euler_rate_rads.z;
}

// Convert an angular velocity vector to a 321-intrinsic euler angle derivative
// Returns false if the vehicle is pitched 90 degrees up or down
bool AC_AttitudeControl::ang_vel_to_euler_rate(const Vector3f& euler_rad,
	const Vector3f& ang_vel_rads, Vector3f& euler_rate_rads) {
float sin_theta = sinf(euler_rad.y);
float cos_theta = cosf(euler_rad.y);
float sin_phi = sinf(euler_rad.x);
float cos_phi = cosf(euler_rad.x);
...
euler_rate_rads.z = (sin_phi / cos_theta) * ang_vel_rads.y
...
Vector3f AC_AttitudeControl::update_ang_vel_target_from_att_error(
	Vector3f attitude_error_rot_vec_rad) {
Vector3f rate_target_ang_vel;
// Compute the roll angular velocity demand from the roll angle error
if (_use_ff_and_input_shaping) {
	rate_target_ang_vel.x = sqrt_controller(attitude_error_rot_vec_rad.x,
			_p_angle_roll.kP(),
			constrain_float(get_accel_roll_max_radss() / 2.0f,
					AC_ATTITUDE_ACCEL_RP_CONTROLLER_MIN_RADSS,
...
if (_use_ff_and_input_shaping) {
	rate_target_ang_vel.y = sqrt_controller(attitude_error_rot_vec_rad.y,
			_p_angle_pitch.kP(),
			constrain_float(get_accel_pitch_max_radss() / 2.0f,
					AC_ATTITUDE_ACCEL_RP_CONTROLLER_MIN_RADSS,
					AC_ATTITUDE_ACCEL_RP_CONTROLLER_MAX_RADSS));
} else {
...
if (_use_ff_and_input_shaping) {
	rate_target_ang_vel.z = sqrt_controller(attitude_error_rot_vec_rad.z,
			_p_angle_yaw.kP(),
			constrain_float(get_accel_yaw_max_radss() / 2.0f,
					AC_ATTITUDE_ACCEL_Y_CONTROLLER_MIN_RADSS,
					AC_ATTITUDE_ACCEL_Y_CONTROLLER_MAX_RADSS));
} else {
...
if (!_motors.limit.roll_pitch
		|| ((integrator > 0 && rate_error_rads < 0)
				|| (integrator < 0 && rate_error_rads > 0))) {
...
float output = get_rate_roll_pid().get_p() + integrator
		...
		+ get_rate_roll_pid().get_ff(rate_target_rads);
...
// Compute output in range -1 ~ +1
float output = get_rate_pitch_pid().get_p() + integrator
		+ get_rate_pitch_pid().get_d()
		+ get_rate_pitch_pid().get_ff(rate_target_rads);
...
// Compute output in range -1 ~ +1
float output = get_rate_yaw_pid().get_p() + integrator
		+ get_rate_yaw_pid().get_d()
		+ get_rate_yaw_pid().get_ff(rate_target_rads);
...
float AC_AttitudeControl::sqrt_controller(float error, float p,
...
float AC_AttitudeControl::stopping_point(float first_ord_mag, float p,
	float second_ord_lim) {
if (second_ord_lim > 0.0f && !is_zero(second_ord_lim) && is_zero(p)) {
...
if (fabsf(first_ord_mag) < linear_velocity) {
	...
	float linear_dist = second_ord_lim / sq(p);
	float overshoot = (linear_dist / 2.0f)
...
return 2.0f * _motors.get_throttle_hover()
		* AC_ATTITUDE_RATE_RP_CONTROLLER_OUT_MAX
		/ ((alpha_remaining * alpha_remaining * alpha_remaining * alpha
				* get_rate_roll_pid().kD()) / _dt + get_rate_roll_pid().kP());
}
...
return 2.0f * _motors.get_throttle_hover()
		* AC_ATTITUDE_RATE_RP_CONTROLLER_OUT_MAX
		/ ((alpha_remaining * alpha_remaining * alpha_remaining * alpha
				* get_rate_pitch_pid().kD()) / _dt + get_rate_pitch_pid().kP());
}
...
return 2.0f * _motors.get_throttle_hover()
		* AC_ATTITUDE_RATE_RP_CONTROLLER_OUT_MAX
		/ ((alpha_remaining * alpha_remaining * alpha_remaining * alpha
				* get_rate_yaw_pid().kD()) / _dt + get_rate_yaw_pid().kP());
}
...
if (_sweep_flag && loopCounterSweep == 0) {
	...
	if (loopCounterSweep < loopCounterLowFreq) {
		...
		kSweep = c2Sweep
				* (expf(
						c1Sweep
								* (((float) loopCounterSweep
										- loopCounterLowFreq) * dtSweep)
								/ (_sweep_length - TrecLowFreq)) - 1);
...
float loopCounterFadeOut = floorf(_sweep_fadeot / dtSweep);

// float _t_park;  %defined in parameters
// float _t_fade_in; %defined in parameters
// float _t_fade_out; %defined in parameters
